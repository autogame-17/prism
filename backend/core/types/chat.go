...
func (r ChatCompletionRequest) ParseToolChoice() (toolType, toolFunc string) {
	if choice, ok := r.ToolChoice.(map[string]any); ok {
		...
		} else if typ, ok := choice["type"].(string); ok && typ == "tool" {
			if name, ok := choice["name"].(string); ok {
				toolType = ToolChoiceTypeFunction
				toolFunc = name
			}
		} else if typ, ok := choice["type"].(string); ok && typ == "any" {
			toolType = ToolChoiceTypeRequired
		}
...
func (r *ChatCompletionRequest) GetFunctions() []*ChatCompletionFunction {
	...
	if r.Tools != nil {
		...
		for _, tool := range r.Tools {
			if tool.Function.Name == "" && tool.Name != "" {
				// Handle Anthropic-formatted tools sent to OpenAI endpoint
				functions = append(functions, &ChatCompletionFunction{
					Name:        tool.Name,
