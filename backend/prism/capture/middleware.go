...
func Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Per-request metadata that does NOT depend on the response.
		// Resolved once here so both the streaming placeholder insert
		// and the non-streaming terminal Save below see identical
		// values without re-parsing the body twice.
		userAgent := extractUserAgent(c.Request)
		sessionID := extractSessionID(c.Request, reqBody)
		cwd := extractCWD(reqBody)
		...
		cacheCreation, cacheRead := extractCacheTokens(body, contentType)
