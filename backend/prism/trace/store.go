...
type Entry struct {
	...
	// SessionID is a best-effort logical session identifier for the
	// client. We try (in order): the X-Session-Id request header, the
	// OpenAI-style `user` body field, and `metadata.session_id`. Empty
	// when no client volunteered one — kept indexed so a future client
	// that does pass an id can be filtered cheaply.
	SessionID string `json:"sessionId" gorm:"type:varchar(96);index"`
	// UserAgent is the verbatim HTTP User-Agent header captured at the
	// moment Prism received the proxied request (e.g. `Cursor/0.x.x`,
	// `python-requests/2.31`). Useful for attributing usage to a
	// specific client build during debugging.
	UserAgent string `json:"userAgent" gorm:"type:varchar(256)"`
	// CWD is the workspace / working-directory hint extracted from the
	// request body when the client embeds one in its system prompt
	// (Cursor uses `Workspace Path: /xxx`). Empty when the request
	// carries no recognisable hint; never used for routing decisions.
	CWD string `json:"cwd" gorm:"type:varchar(512)"`
	// CacheCreationTokens / CacheReadTokens normalise the prompt cache
	// accounting across providers. OpenAI only reports reads
	// (`usage.prompt_tokens_details.cached_tokens`); Anthropic reports
	// both (`cache_creation_input_tokens` / `cache_read_input_tokens`).
	// Both default to 0 when the response lacks the fields, so it is
	// safe to SUM these columns across mixed providers.
...
func List(ctx context.Context, q ListQuery) (*ListResult, error) {
	...
	err := db.Select(
		"id, created_at, request_id, device_id, method, path, status, duration_ms, "+
			...
			"session_id, user_agent, cwd, cache_creation_tokens, cache_read_tokens",
	).Order("id DESC").
